...
let getRetenueForfetaire = () => {
    $('body').on('change', '.retenue-salary', function () {
        ...
        const $amountEmp = $(`#${parentId}_amountEmp`);

        if (retenueForfetaire.length > 0) {
            $amount.val($retenueForfetaire)
            $amountEmp.val($retenueForfetaire)
        } else {
            $amount.val(' ');
            $amountEmp.val(' ')
        }
...
const calculateTotalRetenueForfaitaire = () => {
    ...
    $('.amount-emp-retenue-forfaitaire').each(function () {
        sum1.push(+$(this).val());
    });
    if (sum.length > 0 && sum1.length > 0) {
        total = sum.reduce((previousValue, currentValue) => previousValue + currentValue);
        total2 = sum1.reduce((previousValue, currentValue) => previousValue + currentValue)
    } else {
        total = 0;
        total2 = 0;
    }
    $('#total_montant_retenue_forfaitaire_salary').html(new Intl.NumberFormat('fr-FR').format(total || 0));
    $('#total_montant_retenue_forfaitaire_salaries').html(new Intl.NumberFormat('fr-FR').format(total2 || 0));
}
const retenueForfaitaire = () => {
    $('body').on('change', '.retenue-salary, .amount-retenue-forfaitaire, .amount-emp-retenue-forfaitaire', function () {
        calculateTotalRetenueForfaitaire()
    });
}
