...
class CampagneController extends AbstractController
{
    ...
    public function getPayBook(): JsonResponse
    {
        ...
        foreach ($payroll as $index => $item) {
            ...
            $payBookData[] = [
                ...
                'amount_conges' => $item->getCongesPayesAmount()
            ];
        }
    ...
    public function openCompleteExercice(Request $request, EntityManagerInterface $manager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $dateRequest = $form->get('startedAt')->getData();
            ...
            if ($countPersonal > 0) {
                ...
                $campagne
                    ->setLastCampagne($previousCampagne)
                    ->setActive(true)
        ...
        return $this->render('paiement/campagne/open.html.twig', [
            ...
            'lastCampagne' => $lastCampagneDetail,
            'periode_paie' => $last_periode,
            'today' => new DateTime(),
    ...
    public function findAllBulletin(): Response
    {
        if ($this->isGranted('ROLE_RH')) {
            ...
            $payrolls = $this->payrollRepository->findPayrollByCampaignEmploye(true);
        }
        $pay_book_data = [];
        ...
        foreach ($payrolls as $payroll) {
            ...
            $date_formater = $formatter->format($periode);
            ...
            $years = $periode->format('Y');
            $nb_heure_supp = $this->heureSupRepository->getNbHeursSupp($personal, $month, $years);
            $nb_heure = 0;
            $jour_normal_feri = null;
            $jour_or_nuit = null;
            $amount_hre_sup15 = $amount_hre_sup50 = $amount_hr_sup75A = $amountHeureSup75B = $amountHeureSup100 = null;
            foreach ($nb_heure_supp as $items) {
                $nb_heure += $items?->getTotalHorraire();
                $jour_or_nuit = $items->getTypeJourOrNuit();
                $jour_normal_feri = $items->getTypeDay();
            }
            if ($jour_normal_feri == Status::NORMAL && $jour_or_nuit == Status::JOUR && $nb_heure <= 6) {
                // 15% jour normal ~ 115%
                $amount_hre_sup15 = $payroll->getMajorationAmount();
            } elseif ($jour_normal_feri == Status::NORMAL && $jour_or_nuit == Status::JOUR && $nb_heure > 6) {
                // 50% jour normal ~ 150%
                $amount_hre_sup50 = $payroll->getMajorationAmount();
            } elseif ($jour_normal_feri == Status::DIMANCHE_FERIE && $jour_or_nuit == Status::JOUR) {
                // 75% jour ferié or dimanche jour ~ 175%
                $amount_hr_sup75A = $payroll->getMajorationAmount();
            } elseif ($jour_normal_feri == Status::NORMAL && $jour_or_nuit == Status::NUIT) {
                // 75% jour normal or dimanche nuit ~ 175%
                $amountHeureSup75B = $payroll->getMajorationAmount();
            } elseif ($jour_normal_feri == Status::DIMANCHE_FERIE && $jour_or_nuit == Status::NUIT) {
            ...
            $pay_book_data[] = [
                ...
                'majoration_heure_sup_15' => $amount_hre_sup15,
                'majoration_heure_sup_50' => $amount_hre_sup50,
    ...
    public function bulletinInactif(Payroll $payroll): Response
    {
        $account_banks = [];
        $personal = $payroll->getPersonal();
        $formatter = new IntlDateFormatter('fr_FR', IntlDateFormatter::NONE, IntlDateFormatter::NONE, null, null, "MMMM Y");
        $periode = $payroll->getCampagne()->getDateDebut();
        $date_formater = $formatter->format($periode);
        $account_banques = $payroll->getPersonal()->getAccountBanks();
        foreach ($account_banques as $value) {
            $account_number = $value->getCode() . ' ' . $value->getCodeAgence() . ' ' . $value->getNumCompte() . ' ' . $value->getRib();
            $account_banks[] = [
    ...
    public function ValidatedCampagne(): RedirectResponse
    {
        ...
        if ($campagne) {
            ...
            foreach ($campagne->getPayrolls() as $payroll) {
                $payroll->setStatus(Status::EN_ATTENTE);
            }
    ...
    public function canceledCampagne(): RedirectResponse
    {
        ...
        if ($campagne) {
            $campagne
                ->setLastCampagne(null)
                ->setActive(false)
                ->setStatus(Status::CANCELED);
            $payrolls = $campagne->getPayrolls();
            foreach ($payrolls as $payroll) {
                $payroll->setStatus(Status::CANCELED);
            ...
            foreach ($personals as $personal) {
                ...
                if ($heure_supp) {
                    foreach ($heure_supp as $heure) {
                        $heure->setStatus(Status::EN_ATTENTE);
                        $this->manager->persist($heure);
                    }
                }

                $regularisation = $this->operationRepository->findOperationRegullByPeriode($campagne->getDateDebut(), $campagne->getDateFin(), $personal->getId());
                if ($regularisation) {
                    $regularisation?->setStatus(Status::EN_ATTENTE);
                    $this->manager->persist($regularisation);
                }

                $acomptes_pret = $this->operationRepository->findOperationAcompteAndPreByPeriod($campagne->getDateDebut(), $campagne->getDateFin(), $personal->getId());
                if ($acomptes_pret) {
                    if ($acomptes_pret->getTypeOperations() === Status::ACOMPTE) {
                        $amount = $acomptes_pret->getAmount();
                        $acomptes_pret->setRemaining($amount);
                        $acomptes_pret->setStatusPay(null);
                        $acomptes_pret->setStatus(Status::EN_ATTENTE);
                        $this->manager->persist($acomptes_pret);
                    }
                    if ($acomptes_pret->getTypeOperations() === Status::PRET) {
                        if ($acomptes_pret->getStatusPay() === Status::REFUND) {
                            $amount_mensuality = $acomptes_pret->getAmountMensualite();
                            $amount_remaining = $acomptes_pret->getRemaining();
                            $acomptes_pret->setRemaining($amount_mensuality + $amount_remaining);
                            $acomptes_pret->setStatusPay(null);
                            $acomptes_pret->setStatus(Status::EN_ATTENTE);
                        } else {
                            $amount_mensuality = $acomptes_pret->getAmountMensualite();
    ...
    public function printBulletinEtatSalaire(Request $request): Response
    {
        ...
        if ($request->request->has('printEtatSalaireInput') && $request->isMethod('POST')) {
            ...
            if ($this->isGranted('ROLE_RH')) {
                ...
                $payrolls = $this->payrollRepository->findByPeriodeEmplyees($startAt, $endAt, $personalID);
            }
        }
        $payBookData = [];
        foreach ($payrolls as $payroll) {
    ...
    #[Route('/bulletin/printer/etat/salaire', name: 'printer_bulletin_etat_salaire', methods: ['POST'])]
    public function printer(array $payroll, Request $request): Response
    {
        $payrolls[] = null;
        if ($request->request->has('printEtatSalaireInput') && $request->isMethod('POST')) {
            $payrolls[] = $payroll;
